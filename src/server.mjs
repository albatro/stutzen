...
app.get('/api/supplier/offers', (req, res) => {
  ...
  const rows = db.prepare(`
    SELECT
      ...
      s.count, COALESCE(s.step_quantity, 1) AS step_quantity, s.weight, s.country, s.dimensions, s.available,
...
const SUPPLIER_MINUTE = Number(process.env.SUPPLIER_MINUTE ?? 8);
