...
const table = new Tabulator('#table', {
  ...
  columns: [
    ...
    { title: 'Цена в фиде, ₽', field: 'feed_price', width: 130, hozAlign: 'right',
      formatter: (c) => `<b>${fmtMoney(c.getValue())}</b>`,
      tooltip: 'Цена за единицу × партийность — именно это уходит в XML' },
    { title: 'Цена за ед., ₽', field: 'new_price', width: 120, hozAlign: 'right',
      formatter: (c) => fmtMoney(c.getValue()),
      tooltip: 'Цена за одну единицу товара без учёта партийности' },
    { title: 'Мин. для акций, ₽', field: 'min_for_bestseller', width: 130, hozAlign: 'right',
      formatter: (c) => fmtMoney(c.getValue()), headerSort: false,
      tooltip: 'minimum_price_for_bestseller в XML — равна цене в фиде (с учётом партийности)' },
    ...
    { title: 'Остаток, шт', field: 'count', width: 100, hozAlign: 'right',
      tooltip: 'Остаток от поставщика до деления на партийность' },
    { title: 'Остаток в фиде', field: 'feed_count', width: 110, hozAlign: 'right',
      tooltip: 'Остаток ÷ партийность — именно это уходит в XML' },
