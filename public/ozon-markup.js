...
async function loadRules() {
  const rules = await fetch('/api/ozon/markup-rules').then(r => r.json());
  ...
  tbody.innerHTML = rules.map(r => `
    <tr>
      ...
      <td>${r.scope === 'global' ? '—' : (r.category_name ?? r.ozon_category_id ?? '—')}</td>
      <td>${r.margin_percent}</td>
      <td>${r.min_margin_amount ?? ''}</td>
      <td>${r.active ? 'да' : 'нет'}</td>
      <td>${r.scope === 'category'
        ? `<button class="danger" data-del="${r.id}">Удалить</button>` : ''}</td>
    </tr>
  `).join('');
  ...
  tbody.querySelectorAll('button[data-del]').forEach(b => {
    b.addEventListener('click', async () => {
      ...
      const r = await fetch(`/api/ozon/markup-rules/${b.dataset.del}`, { method: 'DELETE' });
...
document.forms['globalForm'].addEventListener('submit', async (e) => {
  ...
  const r = await fetch('/api/ozon/markup-rules', {
    method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body),
  });
  $('#globalStatus').innerHTML = r.ok
    ? '<span class="ok">сохранено</span>'
    : `<span class="err">${(await r.json()).error}</span>`;
  loadRules();
});
...
document.forms['categoryForm'].addEventListener('submit', async (e) => {
  e.preventDefault();
  const f = e.target;
  if (!f.ozon_category_id.value) {
    $('#categoryStatus').innerHTML = '<span class="err">выбери категорию</span>'; return;
  }
  const body = {
    scope: 'category',
    ozon_category_id: Number(f.ozon_category_id.value),
    margin_percent: Number(f.margin_percent.value),
    min_margin_amount: f.min_margin_amount.value ? Number(f.min_margin_amount.value) : null,
    active: true,
  };
